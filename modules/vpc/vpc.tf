...
resource "aws_vpc" "demovpc" {
  ...
  tags = merge( { "Name" = "${var.env}-vpc"}, local.vpc_common_tags, {"Environment" = "${var.env}"} )
}

resource "aws_internet_gateway" "demo-igw" {
  vpc_id = aws_vpc.demovpc.id
  tags = merge( { "Name" = "${var.env}-igw"}, local.vpc_common_tags, {"Environment" = "${var.env}"} )
}
...
resource "aws_subnet" "demo-pub-subnet" {
 ...
 tags = merge( { "Name" = "${var.env}-${var.network_tags["pub_sub"]}-${count.index + 1}"}, local.vpc_common_tags, {"Environment" = "${var.env}"} )
}
...
resource "aws_subnet" "demo-priv-subnet" {
 ...
 tags = merge( { "Name" = "${var.env}-${var.network_tags["priv_sub"]}-${count.index + 1}"}, local.vpc_common_tags, {"Environment" = "${var.env}"} )
...
resource "aws_route_table" "demo-pub-rt" {
 ...
 tags = merge( { Name = "${var.env}-${var.network_tags["pub_rt"]}" } , local.vpc_common_tags, {"Environment" = "${var.env}"} )
...
resource "aws_route_table" "demo-priv-rt" {
  ...
  tags = merge( { "Name" = "${var.env}-${var.network_tags["priv_rt"]}"}, local.vpc_common_tags, {"Environment" = "${var.env}"} )
