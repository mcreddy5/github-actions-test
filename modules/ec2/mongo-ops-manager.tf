resource "aws_instance" "opsmanager-server" {
    ...
    tags = merge( { "Name" = "${var.instance_tags["mongo_ops"]}" }, local.ec2_common_tags, {"Environment" = "${var.env}"})
...
resource "aws_ebs_volume" "opsmanager-volume" {
  ...
  encrypted = "true"
  tags = merge( { "Name" = "${var.instance_tags["mongo_ops"]}-data-vol"}, local.ec2_common_tags, {"Environment" = "${var.env}"})
}
