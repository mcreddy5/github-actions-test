...
resource "aws_instance" "hazelcast-servers" {
    ...
    tags = merge( { "Name" = "${var.instance_tags["hazelcast"]}-${count.index + 1}" }, local.ec2_common_tags, {"Environment" = "${var.env}"})
    subnet_id = var.priv_sub_id[0] 
    vpc_security_group_ids = [ aws_security_group.hazelcast_sg.id, aws_security_group.common_sg.id ]
}
